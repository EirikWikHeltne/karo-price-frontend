...
export const NON_RETAILER_COLS = new Set([
  'id', 'produktid', 'produkt_id', 'product_id', 'productid', 'produkt', 'merke', 'varenummer', 'kategori',
...
export function deriveRetailers(data) {
  ...
  data.forEach(row => {
    Object.keys(row).forEach(k => {
      ...
      const looksLikeProductId = keyLower === 'produktid' || keyLower === 'productid' || keyLower === 'produkt_id' || keyLower === 'product_id'
      if (!looksLikeProductId && !NON_RETAILER_COLS.has(k) && !seen.has(k)) {
        const val = row[k]
